...
family_for_basename() {
  case "$1" in
    ...
    fm-harness-precedence.test.sh|fm-native-owner-tool-gate.test.sh|\
    ...
    fm-codex-continuity-live-e2e.test.sh|fm-native-owner-codex-live-e2e.test.sh|fm-native-owner-receipt-live-e2e.test.sh|fm-grok-continuity-live-e2e.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-backend.sh|bin/fm-backend-hometag-lib.sh)
      ...
      printf '%s\n' real-herdr-gated
      ;;
    bin/native-owner/*|tests/fixtures/native-owner/*)
