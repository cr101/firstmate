// Interactive native host. The controller has created the private pipe before
// this process resumes. Connect once: EOF or timeout ends this session, never
// reconnect to an endpoint that could now belong to another controller.
...
const evidence={ready:false,turns:[],tools:[],digestDeliveredBeforeDeferred:false};
...
process.on('SIGINT',()=>{if(activeTurn)void interrupt().catch(fail);else void stop();});
process.on('SIGTERM',()=>{void stop();});
const socket=net.createConnection('\\\\.\\pipe\\'+process.env.FM_PROBE_PIPE);
...
const channel=createInterface({input:socket});
...
function native(action,extra={},signal){
 const run=async()=>{
  ...
  return new Promise((resolve,reject)=>{
   ...
   socket.write(JSON.stringify({kind:'notification',session:process.env.FM_PROBE_SESSION,home:runtime,nonce:process.env.FM_PROBE_NONCE,action,...extra})+'\n');
  });
 };
...
try {
 ...
 if(status.operationState==='reconciliation-required')throw Error('An earlier acknowledgement is incomplete. Its records are preserved; review '+path.join(process.env.FM_HOME,'owner-receipts.jsonl')+' before starting more work.');
 ...
 if(!closing){
  ...
  createInterface({input:server.stdout}).on('line',line=>{
   try {
    ...
    if(frame.method==='item/agentMessage/delta'&&frame.params.threadId===thread)process.stdout.write(frame.params.delta);
  ...
  while(!closing&&alive){
   if(process.env.FM_PROBE_VERIFY_ONLY==='1'){
...
}catch(error){if(!closing)fail(error);}finally{await stop();}
